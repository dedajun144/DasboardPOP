...
const SidebarContent = ({ logoText, routes }) => {
  ...
  const createLinks = (routes) => {
    ...
    return routes.map((prop, key) => {
      if (prop.name == "Backbone" || prop.name == "DetailPOP" || prop.name == "S-Backbone" || prop.name == "Distribusi" || prop.name == "Akses" || prop.name == "GI" || prop.name == "Profile") {
        return null;
      }
  ...
  return (
    <>
      {/*   href={`${process.env.PUBLIC_URL}/#/`} target="_blank"*/}
      <Box pt={"25px"} mb="12px">
        <Link href={`/`} display="flex" lineHeight="100%" mb="30px" fontWeight="bold" justifyContent="center" alignItems="center" fontSize="11px">
